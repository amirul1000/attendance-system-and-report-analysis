...
!empty($ATT_UI_DEVICE) && !empty($ATT_UI_BEACON))
{
    ...
    $conn = getConnection();
    $i=0;
    $sqlB = "SELECT ATT_UI_BEACON FROM demosapefficiency.ATTENDANCE WHERE ATT_USER_ID=".$ATT_USER_ID;
        ...
        while($row = $resultb->fetch_assoc()){
            ...
            foreach ($beacon as $value) {
              if(strcmp($value, $ATT_UI_BEACON) == 0){
                //echo "match";
                $sqladd = "SELECT street_number,route,postal_code,country FROM demosapefficiency.ATTENDANCE WHERE ATT_UI_BEACON='".$ATT_UI_BEACON."'";
