...
?>
<a href="index.php?cmd=edit" class="btn green">Add a attendance</a> <br><br>
					         ...
					         <div class="table-responsive">	
				                <table class="table">
									  <TR>
										<TD  nowrap="nowrap">
										  <?php
											  $hash    =  getTableFieldsName("ATTENDANCE");
											  $hash    = array_diff($hash,array("ATT_ID"));
										  ?>
										  Search Key:
										...
										<td nowrap="nowrap" align="right">
										  <input type="hidden" name="cmd" id="cmd" value="search_attendance" >
				   ...
				   <td> 
				      ...
				      <div class="table-responsive">	
						 ...
						 <?php
								...
								$info["fields"] = array("ATTENDANCE.*"); 
								$info["where"]   = "1   $whrstr ORDER BY ATT_ID DESC  LIMIT $offset, $rowsPerPage";
						 ...
						 ?>
							<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
							  <td><?=$arr[$i]['ATT_USER_ID']?></td>
                              <td><?=$arr[$i]['ATT_STR_DATE']?></td>
                              <td><?=$arr[$i]['ATT_END_DATE']?></td>
                              <td><?=$arr[$i]['ATT_WORKING']?></td>
			                  <td><?=$arr[$i]['ATT_LB_DATE']?></td>
                              <td><?=$arr[$i]['ATT_UI_DEVICE']?></td>
                              <td><?=$arr[$i]['ATT_UI_BEACON']?></td>
							  <td nowrap >
								  <a href="index.php?cmd=edit&ATT_ID=<?=$arr[$i]['ATT_ID']?>"  class="btn default btn-xs purple"><i class="fa fa-edit"></i>Edit</a>
								  <a href="index.php?cmd=delete&ATT_ID=<?=$arr[$i]['ATT_ID']?>" class="btn btn-sm red filter-cancel"  onClick=" return confirm('Are you sure to delete this item ?');"><i class="fa fa-times"></i>Delete</a> 
							 </td>
						   ...
						   <td colspan="10" align="center">
							  <?php              
									  unset($info);
									   ...
									   $info["where"]   = "1  $whrstr ORDER BY ATT_ID DESC";
