...
?>
<a href="index.php?cmd=edit" class="btn green">Add a defects</a> <br><br>
	         ...
	         <div class="table-responsive">	
					         ...
					         <div class="table-responsive">	
				                <table class="table">
									  <TR>
										<TD  nowrap="nowrap">
										  <?php
											  $hash    =  getTableFieldsName("DEFECTS");
											  $hash    = array_diff($hash,array("DEFECT_ID"));
										  ?>
										  Search Key:
										...
										<td nowrap="nowrap" align="right">
										  <input type="hidden" name="cmd" id="cmd" value="search_defects" >
			  ...
			  <td>D END DATE</td>
						 ...
						 <?php
								...
								$info["table"] = "DEFECTS";
								$info["fields"] = array("DEFECTS.*"); 
								$info["where"]   = "1   $whrstr ORDER BY DEFECT_ID DESC  LIMIT $offset, $rowsPerPage";
						 ...
						 ?>
							<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
							  <td><?=$arr[$i]['D_USER_ID']?></td>
			  <td><?=$arr[$i]['DEFECT']?></td>
			  <td><?=$arr[$i]['D_ACTION']?></td>
			  <td><?=$arr[$i]['D_DESCRIPTION']?></td>
			  <td><?=$arr[$i]['D_STR_DATE']?></td>
			  <td><?=$arr[$i]['D_END_DATE']?></td>
			  
							  <td nowrap >
								  <a href="index.php?cmd=edit&DEFECT_ID=<?=$arr[$i]['DEFECT_ID']?>"  class="btn default btn-xs purple"><i class="fa fa-edit"></i>Edit</a>
								  <a href="index.php?cmd=delete&DEFECT_ID=<?=$arr[$i]['DEFECT_ID']?>" class="btn btn-sm red filter-cancel"  onClick=" return confirm('Are you sure to delete this item ?');"><i class="fa fa-times"></i>Delete</a> 
							 </td>
						   ...
						   <td colspan="10" align="center">
							  <?php              
									  unset($info);
									   ...
									   $info["where"]   = "1  $whrstr ORDER BY DEFECT_ID DESC";
