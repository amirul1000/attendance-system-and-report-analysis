...
?>
<a href="index.php?cmd=edit" class="btn green">Add a user_confirmation</a> <br><br>
	         ...
	         <div class="table-responsive">	
					         ...
					         <div class="table-responsive">	
				                <table class="table">
									  <TR>
										<TD  nowrap="nowrap">
										  <?php
											  $hash    =  getTableFieldsName("USER_CONFIRMATION");
											  $hash    = array_diff($hash,array("UC_ID"));
										  ?>
										  Search Key:
										...
										<td nowrap="nowrap" align="right">
										  <input type="hidden" name="cmd" id="cmd" value="search_user_confirmation" >
			  ...
			  <td>UC REACTION TIME</td>
						 ...
						 <?php
								...
								$info["fields"] = array("USER_CONFIRMATION.*"); 
								$info["where"]   = "1   $whrstr ORDER BY UC_ID DESC  LIMIT $offset, $rowsPerPage";
						 ...
						 ?>
							<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
							  <td><?=$arr[$i]['UC_DATE']?></td>
			  <td><?=$arr[$i]['UC_USER_ID']?></td>
			  <td><?=$arr[$i]['UC_TIME_DELTA']?></td>
			  <td><?=$arr[$i]['UC_ATT_WORKING']?></td>
			  <td><?=$arr[$i]['UC_ACTION']?></td>
			  <td><?=$arr[$i]['UC_ACTION_CODE']?></td>
			  <td><?=$arr[$i]['UC_DESCRIPTION']?></td>
			  <td><?=$arr[$i]['UC_REACTION_TIME']?></td>
			  
							  <td nowrap >
								  <a href="index.php?cmd=edit&UC_ID=<?=$arr[$i]['UC_ID']?>"  class="btn default btn-xs purple"><i class="fa fa-edit"></i>Edit</a>
								  <a href="index.php?cmd=delete&UC_ID=<?=$arr[$i]['UC_ID']?>" class="btn btn-sm red filter-cancel"  onClick=" return confirm('Are you sure to delete this item ?');"><i class="fa fa-times"></i>Delete</a> 
							 </td>
						   ...
						   <td colspan="10" align="center">
							  <?php              
									  unset($info);
									   ...
									   $info["where"]   = "1  $whrstr ORDER BY UC_ID DESC";
